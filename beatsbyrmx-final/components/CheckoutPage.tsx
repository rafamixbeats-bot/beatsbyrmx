...
import { ShoppingCart, CheckCircleIcon, ArrowDownToLine, DollarSign } from './icons';
...
const CheckoutPage: React.FC<CheckoutPageProps> = ({ items, settings, onConfirmPurchase, onBackToStore }) => {
    ...
    const handleWhatsAppCheckout = () => {
        if (!settings.whatsappNumber) {
            alert('Erro: O número de WhatsApp do vendedor não foi configurado no painel.');
            return;
        }
        ...
        <div className="py-12 md:py-24 animate-fade-in">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto px-4">
                ...
                <div className="space-y-6">
                    <Card className="p-8 border-slate-700 relative overflow-hidden bg-gradient-to-br from-slate-800 to-slate-900">
                         <h2 className="text-2xl font-bold text-slate-100 mb-6">Pagamento via Pix / WhatsApp</h2>
                         <p className="text-slate-300 mb-6">
                             A forma mais rápida e segura. Finalize o pedido pelo WhatsApp e receba a chave Pix. Após o envio do comprovante, os arquivos serão enviados instantaneamente para você.
                         </p>

                         {settings.pixKey && (
                            <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700 mb-6">
                                <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Chave Pix Direta</p>
                         ...
                         <button
                            ...
                            className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-4 px-6 rounded-lg transition-all shadow-lg shadow-green-900/20 active:scale-95 flex items-center justify-center gap-3 text-lg"
                        >
                     ...
                     <button
                        ...
                        className="w-full bg-transparent hover:bg-slate-800 border border-slate-700 text-slate-400 hover:text-white font-bold py-3 rounded-lg transition-colors"
                    >
                        Continuar Comprando
                    </button>
                </div>
